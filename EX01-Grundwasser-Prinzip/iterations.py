...
def main():
    ...
    # We will store history for plotting later
    history_nodes123 = []  # list of [h1, h2, h3] per iteration

    # (Files can still be written if needed, but we focus on plotting)
    ...
    for k in range(solver_iterations):
        ...
        # Store the current values for nodes 1,2,3 (indices 0,1,2)
        history_nodes123.append([x[0], x[1], x[2]])

        # Optional: write to files if needed
        # out_file1.write(f"h1, {x[0]}\nh2, {x[1]}\nh3, {x[2]}\n")
        # out_file2.write(f"h4, {x[3]}\nh5, {x[4]}\nh6, {x[5]}\n")
    ...
    # ------------------------------------------------------------
    # Create individual plots for each iteration (time step)
    # for nodes 1,2,3 (h1, h2, h3) – a line connecting them
    # ------------------------------------------------------------
    node_labels = ['h1', 'h2', 'h3']
    x_pos = [1, 2, 3]

    for idx, h_vals in enumerate(history_nodes123):
        plt.figure(figsize=(6, 5))
        plt.plot(x_pos, h_vals, marker='o', linestyle='-', linewidth=2, markersize=6,
                 color='blue', label=f'Iteration {idx}')
        plt.xticks(x_pos, node_labels)
        plt.xlabel('Node')
        plt.ylabel('Hydraulic head h (m)')
        plt.title(f'Hydraulic heads at nodes 1, 2, 3\nIteration {idx}')
        plt.grid(True)
        plt.legend()
        plt.savefig(f"iter_{idx:03d}.png", dpi=150)
        plt.close()  # close the figure to avoid memory issues

    print(f"Saved {len(history_nodes123)} plot(s) as iter_XXX.png")
